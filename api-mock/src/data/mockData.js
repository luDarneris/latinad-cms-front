var mockData = {
  contents: [
    {
      id: 1,
      name: "Banner verano retail",
      type: "image",
      url: "https://placehold.co/400x300?text=Banner+1",
      category_id: 1,
      folder_id: 3,
      created_at: "2024-01-10",
      archived: false,
      has_audio: false
    },
    {
      id: 2,
      name: "Video promo cine",
      type: "video",
      url: "https://samplelib.com/lib/preview/mp4/sample-5s.mp4",
      category_id: 2,
      folder_id: 5,
      created_at: "2024-01-12",
      archived: false,
      has_audio: true
    },
    {
      id: 3,
      name: "Lanzamiento menu digital",
      type: "image",
      url: "https://placehold.co/400x300?text=Menu+Digital",
      category_id: 1,
      folder_id: 4,
      created_at: "2024-01-15",
      archived: false,
      has_audio: false
    },
    {
      id: 4,
      name: "Trailer estreno",
      type: "video",
      url: "https://samplelib.com/lib/preview/mp4/sample-10s.mp4",
      category_id: 2,
      folder_id: 5,
      created_at: "2024-01-20",
      archived: true,
      has_audio: true
    },
    {
      id: 5,
      name: "Back to school",
      type: "image",
      url: "https://placehold.co/400x300?text=School",
      category_id: 1,
      folder_id: 6,
      created_at: "2024-02-01",
      archived: false,
      has_audio: false
    },
    {
      id: 6,
      name: "Clips temporada verano",
      type: "video",
      url: "https://samplelib.com/lib/preview/mp4/sample-15s.mp4",
      category_id: 3,
      folder_id: 7,
      created_at: "2024-02-04",
      archived: false,
      has_audio: true
    },
    {
      id: 7,
      name: "Promo bebidas",
      type: "image",
      url: "https://placehold.co/400x300?text=Bebidas",
      category_id: 3,
      folder_id: 7,
      created_at: "2024-02-10",
      archived: false,
      has_audio: false
    },
    {
      id: 8,
      name: "Noche de descuentos",
      type: "video",
      url: "https://samplelib.com/lib/preview/mp4/sample-20s.mp4",
      category_id: 1,
      folder_id: 8,
      created_at: "2024-02-14",
      archived: false,
      has_audio: true
    },
    {
      id: 9,
      name: "Campaña bancos",
      type: "image",
      url: "https://placehold.co/400x300?text=Bancos",
      category_id: 4,
      folder_id: 8,
      created_at: "2024-02-19",
      archived: true,
      has_audio: false
    },
    {
      id: 10,
      name: "Cobertura deportiva",
      type: "video",
      url: "https://samplelib.com/lib/preview/mp4/sample-30s.mp4",
      category_id: 2,
      folder_id: 9,
      created_at: "2024-02-25",
      archived: false,
      has_audio: true
    },
    {
      id: 11,
      name: "Nueva colección",
      type: "image",
      url: "https://placehold.co/400x300?text=Coleccion",
      category_id: 1,
      folder_id: 10,
      created_at: "2024-03-02",
      archived: false,
      has_audio: false
    },
    {
      id: 12,
      name: "Invitación concierto",
      type: "video",
      url: "https://samplelib.com/lib/preview/mp4/sample-5mb.mp4",
      category_id: 2,
      folder_id: 10,
      created_at: "2024-03-07",
      archived: false,
      has_audio: true
    },
    {
      id: 13,
      name: "Oferta express",
      type: "image",
      url: "https://placehold.co/400x300?text=Oferta",
      category_id: 3,
      folder_id: 11,
      created_at: "2024-03-11",
      archived: false,
      has_audio: false
    },
    {
      id: 14,
      name: "Spot evento tech",
      type: "video",
      url: "https://samplelib.com/lib/preview/mp4/sample-12s.mp4",
      category_id: 4,
      folder_id: 11,
      created_at: "2024-03-14",
      archived: false,
      has_audio: true
    },
    {
      id: 15,
      name: "Promo feriado",
      type: "image",
      url: "https://placehold.co/400x300?text=Feriado",
      category_id: 1,
      folder_id: 12,
      created_at: "2024-03-18",
      archived: false,
      has_audio: false
    },
    {
      id: 16,
      name: "Video navidad anticipada",
      type: "video",
      url: "https://samplelib.com/lib/preview/mp4/sample-7s.mp4",
      category_id: 3,
      folder_id: 12,
      created_at: "2024-03-22",
      archived: true,
      has_audio: true
    },
    {
      id: 17,
      name: "Branding sucursal centro",
      type: "image",
      url: "https://placehold.co/400x300?text=Sucursal+Centro",
      category_id: 4,
      folder_id: 13,
      created_at: "2024-03-25",
      archived: false,
      has_audio: false
    },
    {
      id: 18,
      name: "Mensaje institucional",
      type: "video",
      url: "https://samplelib.com/lib/preview/mp4/sample-3s.mp4",
      category_id: 4,
      folder_id: 13,
      created_at: "2024-03-28",
      archived: false,
      has_audio: true
    },
    {
      id: 19,
      name: "Pantalla vertical verano",
      type: "image",
      url: "https://placehold.co/400x300?text=Vertical",
      category_id: 2,
      folder_id: 14,
      created_at: "2024-04-02",
      archived: false,
      has_audio: false
    },
    {
      id: 20,
      name: "Cuenta regresiva evento",
      type: "video",
      url: "https://samplelib.com/lib/preview/mp4/sample-4s.mp4",
      category_id: 2,
      folder_id: 14,
      created_at: "2024-04-05",
      archived: false,
      has_audio: true
    }
  ],
  categories: [
    { id: 1, name: "Retail" },
    { id: 2, name: "Entretenimiento" },
    { id: 3, name: "Gastronomía" },
    { id: 4, name: "Servicios" }
  ],
  folders: [
    { id: 1, name: "2024", parent_id: null },
    { id: 2, name: "2025", parent_id: null },
    { id: 3, name: "Verano", parent_id: 1 },
    { id: 4, name: "Otoño", parent_id: 1 },
    { id: 5, name: "Eventos", parent_id: 1 },
    { id: 6, name: "Retail", parent_id: 2 },
    { id: 7, name: "Bebidas", parent_id: 2 },
    { id: 8, name: "Finanzas", parent_id: 2 },
    { id: 9, name: "Deportes", parent_id: 5 },
    { id: 10, name: "Conciertos", parent_id: 5 },
    { id: 11, name: "Tecnología", parent_id: 5 },
    { id: 12, name: "Promos", parent_id: 3 },
    { id: 13, name: "Institucional", parent_id: 4 },
    { id: 14, name: "Pantallas LED", parent_id: 6 }
  ]
};

module.exports = mockData;
